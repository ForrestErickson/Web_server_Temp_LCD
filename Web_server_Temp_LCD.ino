 ...
 *  ESP32 Web server of Temp and Humidity with LCD Display and Button for selection of C/F
   ...
   ESP32 Web Server for DHT22 temperature and humidity sensor, LCD Display and Button by Forrest Lee Erickson   
   ...
   The served web page lights an LED with the temprature is displayed in F
   A physical button changes the display from C to F and back.
   
************************************************************************************
   Operating Instructions:
   Connect DUT to power. 
   Run the Ardiono IDE  and Tools > Serial Monitor or another terminal program on the COM port of the ESP32 device.
   The ESP32 will connect to an AP (access point) with the hard coded SSID and PW.  
   The ESP32 will report the IP address it was assigned to the LAN on the serial monitor. 
   Use a browser to connect to the ESP32 webserver landing page at that IP address.
   The page shows temperature and humidity. You can click on the hyperlinks to control the
   state of the built-in LED (GPIO2) and change temprature reported from C to F.
  ...
  Components
   ----------
    - ESP32 Dev Kit v4, https://www.amazon.com/gp/product/B0718T232Z/ref=ppx_od_dt_b_asin_title_s01?ie=UTF8&psc=1
    - DHT22 temperature and humidity sensor. https://www.amazon.com/gp/product/B01IBBFOF0/ref=ppx_od_dt_b_asin_title_s01?ie=UTF8&psc=1
    - LCD 20x4 with I2C shield, https://www.amazon.com/gp/product/B07HRD8655/ref=ppx_od_dt_b_asin_title_s01?ie=UTF8&psc=1
    - Button SPST momentary
    - 10K leaded resistor
   ...
   -------------------
    ...
    Other information. Notes and some code inherited from example projects at https://github.com/futureshocked/ESP32-For-Busy-People-1
    Example created for arduino 25 Nov 2012   by Tom Igoe, ported for sparkfun esp32 31.01.2017 by Jan Hendrik Berlin
    -----------------
...
//            client.println("<link rel=\"icon\" type=\"image/ico\" sizes=\"16x16\" href=\"data:image/ico;
            ...
            client.println(" %</p>");
            client.println("<p>");           
            //favicon  Use Caution Amused Scientist.
